...
def init_property_of_dataset():
    ...
    trace('OOKB setting, use: different edges')
    aux_link = defaultdict(set)
    file = open(args.auxiliary_file)
    for line in file:
        items = line.strip().split('\t')
        items = list(map(int, items))
        h, r, t = items
    ...
    for mode in ['dev', 'test']:
        ...
        for batch in chunked(current_data, args.test_batch_size):
            with chainer.using_config('train', False), chainer.no_backprop_mode():
                current_score = m.get_scores(batch, train_link, gold_relations, aux_link, xp)
...
def main(args):
    ...
    m.save(f'ookb_gnn_ep{args.epoch_size}.model')

def predict(args, triplet_list):
    init_property_of_dataset()
    load_dataset()
    print('relation size:', args.rel_size, 'entity size:', args.entity_size)

    xp = Backend(args)
    m_load = model.Model(args)
    chainer.serializers.load_npz('ookb_gnn_ep1000.model', m_load)
    # print(m_load)
    with chainer.using_config('train', False), chainer.no_backprop_mode():
        print(m_load.get_scores(triplet_list, train_link, gold_relations, aux_link, xp))
...
def argument():
    ...
    p.auxiliary_file = 'datasets/OOKB/' + p.target_dir + '/aux_'
